...
public class Main2 {
    ...
    static void find_paths(String start ,ArrayList<String> visited, boolean visited_twice, ArrayList<String> path) {
        if (start.equals("end")) {
            paths++;
        ...
        for (String next : adjacency_list.get(start)) {
            if (!visited.contains(next)) {
                ...
                find_paths(next, new_visited, visited_twice, new_path);
            } else {
                if (!visited_twice && !next.equals("start")) {
                    ArrayList<String> new_path = new ArrayList<>(path);
                    new_path.add(next);
                    boolean new_visited_twice = true;
                    find_paths(next, visited, new_visited_twice, new_path);
                }
            }
        }
    }
...
//        }
        ArrayList<String> visited = new ArrayList<>();
        boolean visited_twice = false;
        ArrayList<String> path = new ArrayList<>();
        path.add("start");
        find_paths("start", visited, visited_twice, path);
